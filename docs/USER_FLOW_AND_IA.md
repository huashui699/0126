...
首期体验目标：用户打开网站即看到月历；可在左上角弹窗选择 1–5 支球队，按日期查看三色信息数量、当天摘要和可追溯详情。登录与注册推迟到首版完成后。
...
1. 用户进入首页，系统直接展示当前月份月历；没有本地选择时默认展示全部球队。
2. 用户点击左上角“关注球队”，在弹窗中按联赛选择 1–5 支球队。
3. 用户应用选择后，日历即时只保留相关球队内容，选择保存到浏览器本地。
4. 用户刷新或再次访问时继续使用本地选择。

异常与边界：

- 未选择球队时表示“全部球队”，不会阻断日历浏览。
- 达到 5 支后，其他球队保持可见但不可继续选择，并提示上限。
- 登录、注册、账号同步及游客合并不属于首版范围。

### 4.2 日历浏览与筛选

1. 默认打开当前月份月历，每个日期固定展示绿色、蓝色、红色三类数量。
2. 顶部提供月份切换；左上角提供球队选择弹窗。
3. 用户点击日期后，在月历下方看到当天各条信息的标题、球队、类型、可信度、时间和简述。
4. 日期与月份保留在 URL 中，详情返回时恢复原日期。
5. 某日无结果时明确显示“这一天暂无情报”。
...
作为首次访问的球迷，我希望在日历左上角选择最多 5 支球队，以便快速聚焦相关内容；暂不选择时仍能查看全部内容。

验收标准：

- 弹窗展示固定范围内的 14 支球队。
- 允许选择 1–5 支；0 支表示查看全部球队。
- 超过 5 支不能继续选择，并显示清楚提示。
- 手机端正常流程可在 60 秒内完成。
...
- 当前 V1 只要求浏览器本地保存；账号合并在 V1 后独立验收。
...
作为关注多支球队的用户，我希望先按球队过滤；在可信度引擎完成后，再组合筛选类型和可信度。

验收标准：

- 当前 V1 的球队选择可即时更新结果并在本地恢复。
- 类型、可信度和来源高级筛选在 Day 39 实现，不得提前标为完成。
- Day 39 验收时筛选状态需写入 URL，并提供一键清除。
...
- 当前 V1 的球队选择依赖游客本地存储；`user_team_follows` 保留给 V1 后账号同步。
...
- MVP 默认允许关注 1–5 支球队。
- 默认入口是月历；日期格展示三色数量，日期下方摘要区用于消费内容。
...
- 页面流程是跨端产品语义，不等于只支持 Web；未来 iOS、Android、鸿蒙和微信小程序应复用相同状态、理由码和 API 契约，并采用各平台原生导航。
- 登录注册延后到首版核心日历体验完成后，再恢复账号同步和跨设备关注。
